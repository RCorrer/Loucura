...
async def get_current_user(request: Request) -> Optional[dict]:
    """
    Obtém o usuário atual a partir do cabeçalho OBO (X-Forwarded-Email).
    Valida o perfil no banco usando o cliente simplificado (que retorna listas).
    """
    ...
    try:
        client = get_client()
        # O cliente simplificado retorna uma lista de listas: [[perfil]]
        ...
        if row:
            perfil = row[0]  # acesso por índice, pois row é uma lista
        ...
        # Fallback apenas para desenvolvimento (modo não produção)
