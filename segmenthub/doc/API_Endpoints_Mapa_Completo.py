...
# MAGIC |---|--------|----------|-----------|------------|-----------------|------------------------------|--------------------|-----------------|
# MAGIC | 13 | POST | `/api/segmentacoes` | Cria segmentação (rascunho) | `metadata.catalogo_publicos` (valida publico_base_id) | `segmentacao.seg_definicao` | `seg_id`, `seg_codigo`, `seg_slug`, `nome`, `descricao`, `objetivo`, `owner`, `publico_base_id`, `regras_json`, `status='rascunho'`, `criado_por`, `criado_em` | `SegmentacaoCreateDTO {nome, descricao?, objetivo?, publico_base_id, regras_json?, seg_tags?, owner?, area_responsavel?, email_contato?}` | `{seg_id, seg_codigo, mensagem}` |
