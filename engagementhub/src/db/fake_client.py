"""Fake database client using SQLite for local development (S3 EngagementHub).
...
class FakeSQLiteClient:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self._ensure_db()
        logger.info(f"\u2705 FakeSQLiteClient (S3) inicializado: {self.db_path}")
    ...
    def _normalize_sql(self, sql: str) -> str:
        ...
        Handles:
        - plataforma.engagement.table -> table
        - plataforma.segmentacao.table -> table  (contratos consumidos do S1)
        - plataforma.metadata.table -> table
        - plataforma.governanca.table -> table
        - plataforma.core_cliente.table -> table
        - plataforma.caracteristicas.table -> table
        - plataforma.eventos.table -> table
        ...
        normalized = re.sub(
            r'plataforma\.(engagement|segmentacao|metadata|governanca|'
            r'core_cliente|caracteristicas|publico|eventos)\.',
            '',
            normalized
        )
